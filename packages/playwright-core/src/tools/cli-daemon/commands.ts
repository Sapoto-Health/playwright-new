...
const snapshot = declareCommand({
  ...
  options: z.object({
    ...
    boxes: z.boolean().optional().describe('Include each element\'s bounding box as [box=x,y,width,height] in the snapshot. Defaults to true; pass --no-boxes to omit boxes. Coordinates are viewport-relative, in CSS pixels (Element.getBoundingClientRect).'),
  }),
  toolName: 'browser_snapshot',
